...
export function calculateLongevity(profile: UserProfile): LongevityCalculationResult {
  ...
  // ==========================================
  // Blood Pressure
  const bpCat = profile.bpCategory || (profile.systolicBP !== undefined && profile.diastolicBP !== undefined ? getBloodPressureCategory(profile.systolicBP, profile.diastolicBP) : undefined);
  let bpDelta = 0;
  ...
  let bmiDelta = 0;
  let bmiDisplay = profile.bmi !== undefined ? `BMI ${profile.bmi.toFixed(1)}` : 'Unspecified';
  let bmiExpl = '';
  ...
  // Resting Heart Rate
  let rhrDelta = 0;
  let rhrDisplay = profile.restingHeartRate !== undefined ? `${profile.restingHeartRate} bpm` : 'Unspecified';
  let rhrExpl = '';
  ...
  } else if (profile.polyphenols === 'moderate_coffee_tea') {
    polyDelta = 0.6;
  }
  impacts.push({
    key: 'polyphenols',
  ...
  // Hydration
  let waterDelta = 0;
  let waterDisplay = profile.dailyWaterGlasses !== undefined ? `${profile.dailyWaterGlasses} glasses/day` : 'Unspecified';
  let waterExpl = '';
  if (profile.dailyWaterGlasses !== undefined) {
  ...
  // Sun Protection
  let sunDelta = profile.sunProtection === true ? 0.6 : profile.sunProtection === false ? -0.6 : 0;
  impacts.push({
    key: 'sunProtection',
    label: 'Sun Protection & Skin Health',
    category: 'lifestyle',
    yearsDelta: sunDelta,
    currentValueDisplay: profile.sunProtection === true ? 'Consistent Sunscreen / UV Care' : profile.sunProtection === false ? 'Unprotected UV Exposure' : 'Unspecified',
  ...
  let seatDelta = profile.seatbeltSafety === true ? 0.8 : profile.seatbeltSafety === false ? -2.0 : 0;
  impacts.push({
    key: 'seatbeltSafety',
    label: 'Seatbelt & Transport Safety',
    category: 'lifestyle',
    yearsDelta: seatDelta,
    currentValueDisplay: profile.seatbeltSafety === true ? 'Always Buckled / Safe Driver' : profile.seatbeltSafety === false ? 'Inconsistent Seatbelt / Distracted Driving' : 'Unspecified',
  ...
  let screenDelta = profile.screenings === 'up_to_date' ? 1.8 : profile.screenings === 'neglected' ? -2.0 : 0;
  impacts.push({
    key: 'screenings',
    label: 'Preventative Medical Screenings',
    category: 'lifestyle',
    yearsDelta: screenDelta,
    currentValueDisplay: profile.screenings === 'up_to_date' ? 'Up-to-Date on Screenings' : profile.screenings === 'neglected' ? 'Neglected / Avoids Screenings' : 'Unspecified',
...
export function calculateProgressiveLongevity(
  ...
  const fullProfile: UserProfile = {
    ...answers,
    age: answers.age ?? defaultAge,
    sex: answers.sex ?? defaultSex,
  } as UserProfile;

  return calculateLongevity(fullProfile);
}
